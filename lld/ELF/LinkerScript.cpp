  ...
  for (InputSectionBase *isec : ctx.inputSections) {
    if (isec->name.startswith(".bss_AT") || 
      ...
      if(ATSections.find(isec->name.str()) == ATSections.end())
        ...
        for (InputSectionBase *isecOther : ctx.inputSections)
      ...
      if (config->unique) {
        ...
        if (OutputDesc *os = addInputSec(map, s, name)) {
          if ((config->emachine == EM_RL78) && name.contains("_AT") && (s->flags & SHF_RENESAS_ABS)) {
            ...
            if (nameOffset) {
              uint64_t val;
              std::optional<uint64_t> val1;
              if (!to_integer(name.substr(nameOffset), val, 16))
                val1 = std::nullopt;// TODO: should we output a warning?
              else
                val1 = val;
              ...
              if (val1.has_value() && name.startswith(".const_AT")) {
                if (MemoryRegion *m = memoryRegions.lookup("MIRROR")) {
                  ...
                  if ((mirrorStart > val1.value()) ||
                      (mirrorEnd < val1.value()))
...
void LinkerScript::assignOffsets(OutputSection *sec) {
  ...
  for (SectionCommand *cmd : sec->commands) {
    ...
    if (config->emachine == EM_RL78 && config->strideDSPMemoryArea) {
      ...
      // [s->getParent()->addr + s->outSecOff, s->getParent()->addr +
      // s->outSecOff + s->getSize()) and DSP area by: [0xFD800, 0xFF000).
      if (((state->outSec->addr + isec->outSecOff + isec->getSize() >=
            0xFD800) &&
           (state->outSec->addr + isec->outSecOff < 0xFF000))) {
          dot = alignTo(dot, isec->addralign) + 0xFF000 -
                (state->outSec->addr + isec->outSecOff);
          isec->outSecOff = dot - isec->getSize() - state->outSec->addr;
      }
      }
